...
import com.ex.basic.dto.BasicDto;
...
public class BasicRestController {
    ...
    public ResponseEntity<BasicDto> geMembers(
        ...
        return ResponseEntity.ok(basicDto); // ok : build()까지 포함

    }

    @PostMapping("api/members")
    //fetch(url, {method : post, HttpHeaders : {C-Y : json}, body : JSON.stringify(data)})
    public ResponseEntity<String> postMembers(
            @RequestBody BasicDto basicDto //json 형식으로 처리
    ) {
    ...
    public ResponseEntity<String> postMembersForm(
            @ModelAttribute BasicDto basicDto //form 형식
    ) {
        boolean bool = list.add(basicDto);
        if (bool)
            return ResponseEntity.status(HttpStatus.CREATED).build();
    ...
    public ResponseEntity<Void> putMembers(@ModelAttribute BasicDto basicDto){
        boolean bool = list.stream()
                ...
                .orElse(false);
        if (bool)
            return ResponseEntity.status(HttpStatus.NO_CONTENT).build(); // return으로 돌아오는 값은 없지만 성공했다
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
    }
    ...
    public ResponseEntity<String> getTest01() {
        return ResponseEntity.ok("get 통신 성공");
    }
