...
//                            schema = @Schema(implementation = MemberDto.class)
    ...
    public ResponseEntity<MemberDto> getOne(
            @PathVariable("id") int id // members/0
    ) {
        if (id == 0)
    ...
    public ResponseEntity<MemberDto> getOne2(
            @RequestParam("id") int id // ~members/one?id=0
    ) {
        if (id == 0)
            return ResponseEntity.ok(
                    new MemberDto(0, "aaa", "aaa", "a-role"));
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "409", description = "중복 id",
                    content = @Content(
                            mediaType = "application/json",
                            examples = @ExampleObject(value="동일 ID 문제")
                    )
            )
    })
    public ResponseEntity<String> insert(
            @ParameterObject // swagger form 방식
            @ModelAttribute MemberDto memberDto
    ){
        if (memberDto.getId() == 0)
            return ResponseEntity.status(HttpStatus.CONFLICT).body("중복 id");
        DB.add(memberDto);
